...
El LLM no sabe qué tablas tiene la Empresa A. Para evitar cruces de datos, hemos descentralizado la configuración. No obstante, antes de incorporar las bases de datos de cada cliente a Supabase fue necesario aplicar un proceso de Data Governance. Al tratarse de organizaciones, en muchos casos startups con modelos de datos heterogéneos, desarrollamos scripts de transformación y normalización para homogeneizar tablas, nomenclaturas y tipos de datos, garantizando un esquema consistente sobre el que posteriormente opera el sistema. Una vez normalizados, el frontend Next.js lee archivos estáticos (`tableSchemas.ts` y el fallback `settings.json`) específicos para el `orgId` activo. Esta información (nombres de tablas, tipos de columnas, relaciones) se envía al backend y se inyecta en el "System Prompt" de Ollama. De este modo, la IA genera código SQL puro perfectamente adaptado a la arquitectura de ese cliente.
